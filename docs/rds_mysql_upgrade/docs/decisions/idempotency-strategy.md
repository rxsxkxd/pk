...
### Step 1: precheck — 冪等性は自明に成立。論点は鮮度である
...
実際の課題は冪等性ではなく鮮度である。`metadata.json` は `collected_at` を持つが、**後続の `build_green.sh` はこれを参照しない。** 1 か月前の precheck 結果のまま build を実行できてしまう。
