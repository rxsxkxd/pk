...
| フラグ | 動作 |
|---|---|
| `--check` | 突き合わせを行い、不適合なら終了コード 1 |
| `--output FILE` | Markdown レポートを書き出す |
| 両方 | 検証し、結果をレポートの「0. 検証結果」にも載せる |

検査するのは次の 5 項目である。設定の宣言値は `--expect-*` で渡す。

| 検査 | 宣言値の出どころ | 判定 |
|---|---|---|
| エンジンバージョン | `target_engine_version` | **前方一致**（自動マイナーアップグレードのパッチ差分を許容） |
| インスタンスクラス | `target_db_instance_class` | 完全一致 |
| パラメータグループの関連付け | `target_db_parameter_group_name` | 関連付いていること |
| 適用状態 | — | `in-sync` であること |
| レプリカ遅延 | — | 最大値が 0 秒。**データポイントが無ければ不適合**（確認できていないため） |

これに加えて、宣言値と RDS `Source=user` のドリフトを検出する。

**レポートの内容と終了コードは必ず一致する。**不適合はレポートの「0. 検証結果」に出たうえで、終了コード 1 が返る。
