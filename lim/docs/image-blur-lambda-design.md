...
3. ~~原本と出力で KMS キーを分ける~~ → **同一バケット構成では不可**（§6.1.1）
4. **Lambda はマスク済みの階層に冪等性判定のための `GetObject` しか持たない**（削除は不可）
5. マスク済みの階層は Lambda 実行ロール以外の書き込みを**バケットポリシーで拒否**する
...
- SSE-KMS（カスタマー管理キー。§6.1.1 のとおり 1 つ）
